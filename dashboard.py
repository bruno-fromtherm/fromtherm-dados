...
data_selecionada = st.sidebar.date_input("Filtrar por Data:", value=None, min_value=min(datas_disponiveis) if datas_disponiveis else None, max_value=max(datas_disponiveis) if datas_disponiveis else None)
    ...
    for i, arquivo in enumerate(arquivos_filtrados):
        ...
        with st.expander(expander_title):
            try:
                ...
                st.dataframe(df_info.T, use_container_width=True) # Transpõe para melhor visualização
                ...
                st.download_button(
                    ...
                    data=output_excel,
                    file_name=f"historico_{arquivo['modelo']}_{arquivo['data'].strftime('%Y%m%d')}_{arquivo['hora'].replace(':', '')}.xlsx",
                ...
                pdf_buffer = create_pdf(df_info, df_dados, f"historico_{arquivo['modelo']}_{arquivo['data'].strftime('%Y%m%d')}_{arquivo['hora'].replace(':', '')}.pdf")
                st.download_button(
                    label="Exportar para PDF",
                    data=pdf_buffer,
                    file_name=f"historico_{arquivo['modelo']}_{arquivo['data'].strftime('%Y%m%d')}_{arquivo['hora'].replace(':', '')}.pdf",
